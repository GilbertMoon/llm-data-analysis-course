...
STEP 01~03은 실행 환경 확인, 데이터 로딩, 데이터 구조·품질 관찰 코드와 학생 작성 영역을 제공합니다. STEP 03에서 추가 품질 확인 항목 하나를 학생이 직접 선택합니다. STEP 04~07은 Target 확인, 탐색용 결측 처리, 컬럼 정책 검토, 교육용 인코딩을 제공합니다. 기본안은 수정 없이 순서대로 실행할 수 있으며, 학생은 각 판단란에 자신의 선택과 이유를 기록합니다. `df_work`는 STEP 05에서 한 번 만들고 유지하며, STEP 07의 `df_encoded`는 최종 모델 입력이 아닙니다. STEP 08~17은 기존 주석 골격으로 남아 있습니다.
