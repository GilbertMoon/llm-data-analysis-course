...
CSV 파일을 pandas로 불러오는 일 자체는 어렵지 않습니다. 하지만 파일을 불러왔다고 해서 곧바로 분석을 시작할 수 있는 것은 아닙니다. 날짜가 문자열로 저장되어 있을 수도 있고, 숫자처럼 보이는 값이 실제로는 문자일 수도 있습니다. 어떤 컬럼에는 값이 비어 있을 수 있고, 어떤 ID는 중복되어 있을 수도 있습니다. 이런 부분을 확인하지 않고 분석을 시작하면 나중에 그래프나 모델 결과가 그럴듯해 보여도 잘못된 결론으로 이어질 수 있습니다.

이번 장에서는 온라인 쇼핑몰 고객·매출 분석 프로젝트에서 사용할 4개의 CSV 파일을 불러오고, 데이터의 기본 구조를 읽는 방법을 살펴봅니다. 이 과정은 이후 pandas 기초, 전처리, EDA, 시각화, 머신러닝, LLM 기반 분석으로 이어지는 출발점입니다.
...
| 데이터 타입 | 숫자, 문자, 날짜 등 값의 종류를 확인합니다. |
| 결측치 | 비어 있는 값이 있는지 확인합니다. |
| 중복 데이터 | 같은 행이나 같은 ID가 반복되는지 확인합니다. |
| 기본 통계 | 숫자형 컬럼의 범위와 이상 가능성을 살펴봅니다. |
| 키 컬럼 | 여러 파일을 연결할 기준 컬럼을 확인합니다. |

이 과정은 지루해 보일 수 있지만, 실제 분석에서 가장 중요한 습관 중 하나입니다. 데이터를 제대로 확인한 사람은 이후 전처리와 시각화, 머신러닝 단계에서 발생할 오류를 훨씬 빠르게 발견할 수 있습니다.

## 2. CSV 파일과 DataFrame

CSV는 Comma-Separated Values의 줄임말입니다. 말 그대로 값을 쉼표로 구분해 저장한 텍스트 파일입니다. 메모장으로 열면 쉼표로 구분된 텍스트처럼 보이고, 엑셀로 열면 표처럼 보입니다.

pandas는 CSV 파일을 읽어 `DataFrame`이라는 표 형태의 데이터로 다룹니다. DataFrame은 엑셀 시트와 비슷하게 행과 열로 구성되어 있습니다.

| 구성 요소 | 설명 |
| --- | --- |
| 행(row) | 하나의 관측값 또는 기록입니다. |
| 열(column) | 데이터의 속성 또는 변수입니다. |
| 인덱스(index) | 각 행을 구분하는 번호 또는 이름입니다. |
...
처음에는 이 파일들을 따로 살펴봅니다. 각 파일이 어떤 구조인지 이해한 뒤에야 고객별 구매금액, 상품 카테고리별 매출, 주문 취소율 같은 분석으로 확장할 수 있습니다.
...
이번 장의 코드는 `notebooks/ch03_data_overview.ipynb`에서 실행할 수 있습니다. 여기서는 핵심 흐름만 살펴봅니다.
...
CSV 파일을 불러올 때 가장 흔한 오류는 파일 경로 오류입니다. 먼저 현재 작업 폴더를 확인해 봅니다.
...
Notebook을 `notebooks` 폴더 안에서 실행하는 경우에는 데이터 경로가 달라질 수 있습니다. 파일을 찾지 못하는 오류가 발생하면 현재 작업 폴더를 먼저 확인하는 습관을 들이는 것이 좋습니다.

### 데이터 폴더 경로 설정하기

프로젝트 루트에서 실행하는 경우를 기준으로 데이터 경로를 설정합니다.
...
하나라도 `False`가 나오면 경로가 잘못되었거나 샘플 데이터가 아직 생성되지 않은 것입니다. 이 경우 `python scripts/generate_sample_data.py`를 먼저 실행해 데이터 파일을 생성합니다.
...
정상적으로 실행되면 각 파일이 pandas DataFrame으로 저장됩니다. 이제부터는 이 변수들을 사용해 데이터 구조를 확인할 수 있습니다.
...
`shape` 결과는 `(행 개수, 열 개수)` 형식입니다. 예를 들어 `(150, 6)`은 150개의 행과 6개의 열이 있다는 뜻입니다. 실제 숫자는 샘플 데이터 생성 방식에 따라 달라질 수 있습니다.

여러 데이터의 크기를 하나의 표로 정리하면 전체 구조를 보기 쉽습니다.
...
이 표는 보고서나 발표 자료에서 데이터 개요를 설명할 때도 자주 사용됩니다.
...
앞부분만 보고 전체 데이터가 정상이라고 판단하기는 어렵습니다. `tail()`로 마지막 부분도 확인해 보면 데이터가 끝까지 비슷한 구조를 유지하는지 점검할 수 있습니다.
...
예를 들어 실제 컬럼은 `customer_id`인데 LLM이 `cust_id`라고 코드를 작성하면 실행되지 않습니다. 따라서 LLM의 도움을 받더라도 실제 컬럼명 확인은 사람이 해야 합니다.
...
데이터 구조를 볼 때 가장 먼저 확인해야 할 문제는 결측치와 중복입니다. 결측치는 값이 비어 있는 상태이고, 중복은 같은 값이나 같은 행이 반복되는 상태입니다. 둘 다 분석 결과에 영향을 줄 수 있지만, 무조건 삭제해야 하는 것은 아닙니다.
...
결측치 비율을 확인하면 컬럼별로 얼마나 많은 값이 비어 있는지 더 쉽게 이해할 수 있습니다.
...
결과가 `0`이면 해당 컬럼에 결측치가 없다는 뜻입니다. 결측치 비율이 높다면 이후 전처리 단계에서 처리 방법을 결정해야 합니다. 예를 들어 `age` 컬럼에 결측치가 있으면 연령대별 분석에서 일부 고객이 제외될 수 있습니다.

결측치를 처리하는 방법은 분석 목적에 따라 달라집니다.
...
| 컬럼 제외 | 분석에 적합하지 않은 컬럼은 사용하지 않습니다. |
...
`customers["customer_id"]`가 중복되면 고객 정보가 중복 저장되었을 가능성이 있습니다. 반면 `order_items["order_id"]`가 중복되는 것은 자연스러울 수 있습니다. 중복은 숫자만 보고 판단하지 말고, 데이터의 의미와 함께 해석해야 합니다.
...
최댓값이나 최솟값이 지나치게 크거나 작으면 이상치 가능성을 의심할 수 있습니다. 평균과 중앙값의 차이가 크면 값의 분포가 한쪽으로 치우쳐 있을 수도 있습니다.
...
`errors="coerce"`는 숫자로 바꿀 수 없는 값이 있을 때 오류를 내지 않고 `NaN`으로 처리합니다. 변환 후에는 결측치가 새로 생겼는지도 함께 확인해야 합니다.

주문 상세 데이터에서 수량과 단가도 확인합니다.
...
`nunique()`는 서로 다른 값이 몇 개인지 알려 줍니다. `value_counts()`는 각 값이 몇 번 등장했는지 보여 줍니다.
...
`isin()`은 특정 컬럼의 값이 다른 컬럼의 값 목록 안에 포함되어 있는지 확인합니다. 앞에 붙은 `~`는 조건을 반대로 바꾸는 의미입니다. 따라서 위 코드는 “연결 대상 테이블에 존재하지 않는 ID”만 골라내는 검사입니다.
...
`def`는 사용자가 직접 함수를 만들 때 사용하는 Python 문법입니다. 위 함수는 데이터프레임 이름과 데이터프레임을 입력받아 shape, 컬럼, 데이터 타입, 결측치, 중복 행을 한 번에 출력합니다.
...
이 함수는 간단하지만 실무에서도 유용한 기본 패턴입니다. 새로운 데이터를 받을 때마다 비슷한 방식으로 구조를 점검하면 분석 초기에 발생하는 실수를 줄일 수 있습니다.

## 8. LLM에게 데이터 구조를 설명시키는 법

LLM은 데이터 구조를 이해하고 분석 계획을 세우는 데 도움을 줄 수 있습니다. 하지만 원본 데이터를 그대로 붙여 넣는 방식은 피해야 합니다. 고객명, 주문 상세, 실제 거래 정보처럼 개인이나 거래를 식별할 수 있는 값은 제외하고, 컬럼명, 데이터 타입, 행과 열 개수, 결측치 요약, 중복 여부처럼 구조를 설명하는 정보만 입력하는 것이 안전합니다.

예를 들어 `customers.head()` 전체 결과를 그대로 입력하기보다 다음처럼 요약해서 질문할 수 있습니다.
...
이 방식은 개인정보와 원본 데이터를 보호하면서도 LLM의 도움을 받을 수 있는 방법입니다.
...
LLM이 제안한 분석이 실제 데이터와 맞는지 확인하는 과정은 반드시 필요합니다. 예를 들어 LLM이 연령대별 매출 분석을 제안했다면 `age` 컬럼이 실제로 존재하는지, 결측치가 있는지, 고객 데이터와 주문 데이터가 `customer_id`로 연결되는지 확인해야 합니다.

### 데이터 구조 요약 보고서 작성 요청 예시
...
LLM은 초안을 빠르게 만들어 줄 수 있지만, 보고서에 들어가는 숫자와 해석은 반드시 실제 실행 결과를 기준으로 확인해야 합니다.

## 9. 구조 점검에서 다음 분석으로

이번 장에서 확인한 결과는 최종 분석 결론이 아닙니다. 데이터 분석을 시작하기 전의 사전 점검 결과입니다. 데이터가 몇 행과 몇 열인지, 어떤 컬럼이 있는지, 결측치와 중복이 있는지, 파일 간 키 관계가 유지되는지를 확인해야 이후 분석이 안정적으로 이어집니다.

데이터 구조 점검을 마칠 때는 다음 항목을 간단히 확인해 봅니다.
...
- 각 파일의 결측치와 중복 여부를 확인합니다.
- 주문일 컬럼을 날짜 타입으로 변환하고 데이터 기간을 확인합니다.
- LLM에게 데이터 구조 요약을 요청한 뒤, 실제 데이터와 맞는지 검증합니다.
- `check_data_overview()` 함수에 결측치 비율과 데이터 타입 요약을 추가합니다.
