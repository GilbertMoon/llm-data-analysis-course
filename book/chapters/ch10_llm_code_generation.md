...
- 검증 데이터에서 Logistic Regression과 Random Forest를 비교했고, 선택한 모델과 임계값을 테스트 데이터에서 최종 평가했습니다.
- 취소 주문 탐지 성능을 확인하기 위해 accuracy뿐 아니라 precision, recall, f1-score와 혼동행렬을 함께 확인했습니다.
