...
`std`는 값이 평균에서 얼마나 퍼져 있는지를 나타내는 표준편차입니다. `25%`, `50%`, `75%`는 데이터를 작은 값부터 큰 값까지 정렬했을 때의 사분위수입니다. 특히 `50%`는 중앙값이며, 평균과 함께 데이터의 중심을 이해하는 데 사용합니다.
...
`nunique()`는 서로 다른 값이 몇 개인지 개수만 알려 줍니다. `value_counts()`는 각 값이 몇 번 등장했는지 빈도까지 보여 줍니다. 따라서 그룹의 종류만 알고 싶을 때는 `nunique()`, 특정 값이 얼마나 많은지 보고 싶을 때는 `value_counts()`를 사용합니다.
...
`datasets`는 데이터 이름과 DataFrame을 짝지어 저장한 딕셔너리입니다. `.items()`를 사용하면 이름과 DataFrame을 하나씩 꺼내 반복 처리할 수 있습니다.
