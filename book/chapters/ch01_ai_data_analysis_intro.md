...
- 분석 과정에서 프롬프트 로그와 데이터 보안이 중요한 이유를 이해할 수 있습니다.
...
데이터 분석은 보통 문제 정의에서 시작해 데이터 수집, 구조 확인, 전처리, 탐색적 분석, LLM 활용 보조, 인사이트 도출, 보고서 작성으로 이어집니다. 아래 그림은 이 장에서 이해해야 할 전체 흐름을 요약한 것입니다.
...
| 어떤 고객군이 많이 구매하는가? | 성별, 연령대, 지역별 구매금액을 비교한다. |
...
데이터 분석에서는 표 형태의 데이터를 다룰 때 pandas를 자주 사용합니다. 일반적으로 `pd`라는 짧은 이름으로 불러옵니다.
...
열 개수는 데이터가 가진 정보의 종류를 알려줍니다. 고객 데이터에 `age`, `gender`, `city`가 있다면 연령, 성별, 지역별 분석이 가능할 수 있습니다. 하지만 이 컬럼들이 없다면 해당 분석은 바로 할 수 없습니다.
