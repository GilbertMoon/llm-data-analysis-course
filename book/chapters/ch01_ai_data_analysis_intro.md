...
이 책의 실습은 하나의 프로젝트 폴더를 기준으로 진행하는 것이 좋습니다. 프로젝트 폴더는 단순히 파일을 모아 두는 공간이 아니라, 코드와 데이터, Notebook, 보고서, 환경 설정을 함께 관리하는 작업 공간입니다.
...
| `notebooks/` | 장별 실습 Notebook | 코드를 실행하고 결과와 해석 기록 |
...
| `README.md` | 프로젝트 안내 | 설치, 실행, 작업 흐름 확인 |

분석을 진행하다 보면 파일이 계속 늘어납니다. 이때 폴더 역할을 지키면 나중에 결과를 찾기 쉽고, 다른 사람에게 프로젝트를 설명하기도 쉬워집니다.

## 10. 실습을 시작하는 기본 흐름

이 장에서는 실습을 자세히 진행하기보다, 앞으로 어떤 순서로 실습이 이어지는지만 먼저 확인합니다. 실제 VS Code 실행, 가상환경 생성, 패키지 설치, Notebook 커널 선택, 첫 셀 실행 확인은 **2장. VS Code에서 시작하는 데이터 분석 환경**에서 단계별로 진행합니다.

1장에서 기억해야 할 핵심은 다음과 같습니다.

- 실습은 하나의 프로젝트 폴더를 기준으로 진행합니다.
- 데이터는 `data/` 폴더, Notebook은 `notebooks/` 폴더, 보고서는 `reports/` 폴더에 정리합니다.
- Python 패키지는 프로젝트별 가상환경인 `.venv` 안에 설치합니다.
- Notebook을 실행할 때는 VS Code에서 `.venv` 커널을 선택해야 합니다.
...
## 12. 개인정보와 API Key는 프로젝트에 포함하지 않는다
