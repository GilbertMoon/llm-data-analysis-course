...
대규모 언어 모델(LLM)의 등장은 이 과정을 크게 바꾸고 있습니다. ChatGPT나 Gemini 같은 도구는 분석 질문을 정리하고, pandas 코드 초안을 작성하고, 오류 메시지를 설명하고, 그래프 해석이나 보고서 문장을 다듬는 데 도움을 줄 수 있습니다. 그러나 분석의 결론을 책임지는 주체는 여전히 사람입니다. AI는 분석가를 대신한다기보다, 분석가가 더 빠르게 생각하고 더 넓게 검토하도록 돕는 도구에 가깝습니다.

이 책은 Python 데이터 분석의 기본 과정을 익힌 뒤, 간단한 머신러닝과 LLM 활용, 외부 데이터 수집, 보고서 작성, 자동화 개념까지 자연스럽게 확장하도록 구성되어 있습니다. 하나의 온라인 쇼핑몰 데이터를 바탕으로 탐색적 데이터 분석(EDA), 시각화, 회귀·분류 모델, LLM 프롬프트, 보고서, 자동화 아이디어까지 연결해 봅니다.

이 장은 본격적인 코딩에 들어가기 전의 안내판입니다. 데이터 분석을 어떤 관점으로 바라봐야 하는지, LLM을 어디까지 믿고 어디서부터 검증해야 하는지, 그리고 실습 환경을 어떻게 준비하면 좋은지 먼저 정리합니다.
...
- 실습 프로젝트는 어떤 폴더 구조로 정리하면 좋을까?
- 개인정보와 API 키는 어떻게 보호해야 할까?
...
| 목적 | 결과를 어디에 쓸 것인가? | 매출 변화 요인 파악, 캠페인 기획, 재고 관리, 고객 관리 |
...
이렇게 질문을 바꾸면 `quantity`, `unit_price`, `product_id`, `product_name` 같은 컬럼이 필요하다는 점이 분명해집니다. 총매출을 계산할 때는 `quantity`와 `unit_price`를 곱해 `line_total`과 같은 파생 컬럼을 만들 수도 있습니다.
...
이 책에서는 여기에 두 가지 요소를 더합니다. 하나는 머신러닝입니다. EDA와 시각화가 “무슨 일이 있었는가”를 이해하는 데 가깝다면, 머신러닝은 “앞으로 어떤 일이 일어날 가능성이 있는가”를 예측하는 방향으로 확장됩니다. 다른 하나는 자동화입니다. 반복되는 분석 과정을 보고서 생성이나 파이프라인 형태로 연결하면 분석 결과를 더 안정적으로 재사용할 수 있습니다.
...
이 순서는 이 책의 학습 구성을 보여 줍니다. 실제 분석에서 LLM 활용은 반드시 한 번만 거치는 독립 단계라기보다, 질문 정리, 코드 작성, 오류 해결, 결과 해석 등 여러 단계에서 선택적으로 활용하는 보조 활동에 가깝습니다.
...
| 머신러닝 | 예측할 수 있는가? | 회귀, 분류, 훈련/테스트 데이터 분할, 평가 지표 |
...
외부 데이터나 크롤링 결과를 사용할 때는 제공 기관과 웹사이트의 이용약관, 저작권, 개인정보 보호 기준을 먼저 확인해야 합니다.

중요한 점은 LLM이 분석 과정의 일부를 빠르게 도와줄 수는 있지만, 분석 질문을 정의하고 최종 결과를 검증하는 책임은 사람에게 있다는 것입니다.

## 3. LLM은 데이터 분석을 어떻게 바꾸는가

LLM을 잘 활용하면 분석을 시작하는 속도를 높일 수 있습니다. 특히 처음 데이터를 받았을 때 “무엇부터 봐야 할지” 막막한 상황에서 유용합니다.

예를 들어 온라인 쇼핑몰 데이터가 있다고 할 때, 다음과 같은 요청을 할 수 있습니다.

```text
온라인 쇼핑몰의 고객, 상품, 주문, 주문 상세 데이터가 있습니다.
...
하지만 LLM의 답변은 분석의 결론이 아니라 출발점입니다. LLM은 데이터의 업무 맥락과 품질을 스스로 보장하지 못하며, 사용자가 제공한 설명, 일부 샘플, 일반적인 패턴을 바탕으로 그럴듯한 답을 생성할 수 있습니다. 따라서 답변이 자연스럽게 보인다고 해서 곧바로 맞는 분석이라고 판단해서는 안 됩니다.
...
| 코드 작성 | pandas·시각화·머신러닝 코드 초안 | 컬럼명, 데이터 타입, 실행 결과 검증 |
| 오류 해결 | 오류 메시지 원인 설명 | 실제 환경과 파일 경로 확인 |
| 해석 작성 | 보고서 문장 초안 | 데이터에 없는 원인 단정 제거 |
| 발표 준비 | 발표 구성과 문장 정리 | 핵심 수치와 근거 확인 |
...
- 결측치나 이상값이 분석에 미치는 영향 판단하기
- 회사나 프로젝트의 내부 맥락 이해하기
- 머신러닝 모델의 성능이 충분한지 판단하기
- 그래프와 보고서 해석이 데이터보다 과장되지 않았는지 확인하기
- 개인정보와 민감정보를 안전하게 처리하기

예를 들어 LLM이 “고객 연령대별 매출을 분석하세요”라고 제안했다고 해도, 실제 `customers.csv`에 `age` 컬럼이 없다면 그 분석은 바로 수행할 수 없습니다. 이 경우 연령대 분석을 제외하거나, 적법하게 사용할 수 있는 다른 데이터에서 나이 정보를 가져올 수 있는지 확인해야 합니다.
...
이 책에서는 하나의 가상 온라인 쇼핑몰 데이터를 중심으로 분석을 진행합니다. 데이터는 `Faker` 라이브러리로 생성했으며, 실제 개인과 무관한 실습용 데이터입니다.
...
- 어떤 상품 카테고리가 잘 팔리는가?
- 어떤 고객군이 많이 구매하는가?
- 매출 감소는 어떤 지표 변화와 함께 나타나는가?
- 주문이 취소될 가능성이 높은 조건은 무엇인가?
- 고객의 향후 구매 금액이나 매출을 예측할 수 있는가?
- 어떤 마케팅 방안을 제안할 수 있는가?

처음에는 단순한 집계와 시각화로 시작하지만, 뒤로 갈수록 머신러닝과 LLM 활용으로 확장합니다. 예를 들어 회귀 모델로 고객의 과거 구매 이력 등을 바탕으로 향후 구매 금액을 예측하고, 분류 모델로 주문 취소 가능성이나 구매 가능성을 예측해 볼 수 있습니다. 이후 LLM을 활용해 분석 질문을 정리하고, 코드 초안을 생성하고, 결과 해석과 보고서 문장을 다듬는 방식으로 연결합니다.
...
파일을 연결한다는 것은 두 표에서 같은 의미를 가진 기준 컬럼을 사용해 하나의 분석용 표로 합친다는 뜻입니다. 예를 들어 고객별 구매 금액을 분석하려면 `customers.csv`, `orders.csv`, `order_items.csv`를 연결해야 합니다. 상품 카테고리별 매출을 분석하려면 `products.csv`와 `order_items.csv`를 연결해야 합니다.
...
초기 데이터 분석에서는 주로 “무슨 일이 있었는가”를 확인합니다. 월별 매출이 늘었는지 줄었는지, 어떤 카테고리가 많이 팔렸는지, 어떤 고객군의 구매 금액이 높은지 살펴봅니다. 이런 과정을 탐색적 데이터 분석, 즉 EDA라고 부릅니다.

머신러닝은 여기에서 한 걸음 더 나아갑니다. 이미 발생한 데이터를 바탕으로 새로운 데이터의 값을 예측하거나, 특정 상태에 속할 가능성을 판단합니다.

이 책에서 다룰 머신러닝 실습은 복잡한 이론보다 분석 과정을 이해하는 데 초점을 둡니다.

| 구분 | 예시 질문 | 대표 모델 |
| --- | --- | --- |
| 회귀 | 고객의 향후 구매 금액을 예측할 수 있을까? | Linear Regression, RandomForestRegressor |
| 분류 | 주문이 취소될 가능성이 높은가? | Logistic Regression, RandomForestClassifier |

회귀는 숫자를 예측하는 문제입니다. 예를 들어 고객의 과거 구매 이력, 가입 기간, 최근 구매 시점처럼 예측 시점 이전에 확인할 수 있는 정보를 바탕으로 향후 구매 금액을 예측할 수 있습니다. 분류는 정해진 범주를 예측하는 문제입니다. 예를 들어 주문 상태가 `completed`인지 `cancelled`인지 예측하는 문제를 생각해 볼 수 있습니다.
...
프롬프트 로그에는 모든 대화를 길게 복사할 필요는 없습니다. 분석에 영향을 준 주요 질문, LLM의 핵심 답변, 실제 반영 여부를 간단히 적어 두면 충분합니다. 단, 로그에도 개인정보, API 키, 내부 기밀정보를 남기지 않아야 합니다.
...
실제 고객 이름, 전화번호, 이메일, 주소, 결제 정보 같은 개인정보를 LLM에 그대로 입력하면 안 됩니다. 회사 내부 매출 자료, 계약 정보, 비공개 전략 자료도 조직에서 사용을 승인하지 않은 외부 LLM 서비스에 입력해서는 안 됩니다.

실무에서는 다음 원칙을 지켜야 합니다.

- 개인정보는 제거하거나 비식별화합니다.
- 내부 기밀 데이터는 승인되지 않은 외부 LLM 서비스에 입력하지 않습니다.
- 필요한 경우 전체 데이터가 아니라 일부 샘플 구조나 요약 정보만 제공합니다.
- API 키, 비밀번호, 토큰은 절대 프롬프트에 넣지 않습니다.
- 조직의 보안 정책과 허용된 도구를 먼저 확인합니다.
...
이 정도 정보만으로도 LLM은 분석 방향이나 코드 초안을 제안할 수 있습니다. 필요한 정보만 최소한으로 제공하는 습관은 AI 시대의 기본 데이터 보안 원칙입니다.

## 9. 실습 프로젝트는 어떻게 구성할까

이 책의 실습은 하나의 프로젝트 폴더를 기준으로 진행하는 것이 좋습니다. 프로젝트 폴더는 단순히 파일을 모아 두는 공간이 아니라, 코드와 데이터, Jupyter Notebook, 보고서, 환경 설정을 함께 관리하는 작업 공간입니다.

실습 프로젝트에서 자주 사용하게 될 폴더와 파일은 다음과 같습니다.
...
| 위치 | 역할 | 사용 예시 |
| --- | --- | --- |
| `data/raw/` | 원본 또는 샘플 CSV 저장 | 생성된 CSV 확인 |
| `data/processed/` | 전처리된 CSV 저장 | 결측치·타입 정리 후 정제된 파일 저장 |
| `data/external/` | 외부 데이터 저장 | 공공데이터, API, 크롤링 결과 저장 |
| `notebooks/` | 장별 실습 노트북 | 코드를 실행하고 결과와 해석 기록 |
...
| `.env.example` | 환경변수 예시 | API 키 변수 이름을 확인한 뒤 `.env` 작성 |
| `.gitignore` | Git에서 제외할 파일 목록 | `.env`, `.venv`, 캐시 파일 제외 확인 |
| `requirements.txt` | 필요한 패키지 목록 | 가상환경에 패키지 설치 |
| `README.md` | 프로젝트 안내 | 설치, 실행, 작업 과정 확인 |

분석을 진행하다 보면 파일이 계속 늘어납니다. 이때 폴더 역할을 지키면 나중에 결과를 찾기 쉽고, 다른 사람에게 프로젝트를 설명하기도 쉬워집니다.

### Git과 GitHub 사전 학습 안내

이 책의 실습 파일과 변경 이력을 안정적으로 관리하려면 Git과 GitHub의 기본 개념을 함께 익혀 두는 것이 좋습니다.

Git과 GitHub의 차이, Git 설치, 저장소 생성, Clone, Add, Commit, Push, Pull 등 기본적인 설치 및 사용 방법은 다음 가이드를 참고합니다.

- [Git·GitHub 개념, 설치 및 기본 사용 가이드](https://blog.naver.com/dev-dog/223840788612)

이 책에서는 Git과 GitHub 자체를 자세히 설명하기보다, 준비된 저장소를 내려받고 실습 코드와 분석 결과의 변경 이력을 관리하는 용도로 활용합니다.

단, `.env`, API 키, 비밀번호, 개인정보, 회사 내부 데이터와 대용량 원본 데이터는 Git 저장소에 커밋하지 않도록 주의해야 합니다.

## 10. 실습을 시작하는 기본 흐름

이 장에서는 실습을 자세히 진행하기보다, 앞으로 어떤 순서로 실습이 이어지는지만 먼저 확인합니다. 실제 VS Code 실행, 가상환경 생성, 패키지 설치, Jupyter Notebook 커널 선택, 첫 셀 실행 확인은 **2장. VS Code에서 시작하는 데이터 분석 환경**에서 단계별로 진행합니다.

1장에서 기억해야 할 핵심은 다음과 같습니다.

- 실습은 하나의 프로젝트 폴더를 기준으로 진행합니다.
- 데이터는 `data/` 폴더, 노트북은 `notebooks/` 폴더, 보고서는 `reports/` 폴더에 정리합니다.
- Python 패키지는 프로젝트별 가상환경인 `.venv` 안에 설치합니다.
- 노트북을 실행할 때는 VS Code에서 `.venv` 커널을 선택해야 합니다.
- 파일 경로 오류가 나면 현재 작업 폴더와 데이터 파일 위치를 함께 확인해야 합니다.

따라서 지금 단계에서는 “이후 장에서 직접 실행할 분석 프로젝트가 어떤 구조로 준비되는지”를 이해하는 데 집중합니다. 실제로 `notebooks/ch01_ai_data_analysis_intro.ipynb`를 열고 첫 번째 셀을 실행하는 과정은 2장에서 화면 흐름과 함께 다시 안내합니다.

## 11. LLM과 함께 실습할 때의 좋은 질문 예시

실습하다 보면 파일 경로 오류, 패키지 오류, 컬럼명 오류, 그래프 한글 깨짐 같은 문제가 자주 발생합니다. 이때 LLM에게 질문할 수 있지만, 질문에는 상황을 구체적으로 담아야 합니다.
...
VS Code의 Jupyter Notebook에서 실행 중입니다.
...
## 12. API 키와 비밀정보는 Git 저장소에 포함하지 않는다

11장 이후에는 LLM API나 외부 API를 활용하는 과정이 등장합니다. 이때 `.env.example` 파일을 복사해 `.env` 파일을 만들고, 실제 키는 `.env`에만 저장합니다. `.env.example`에는 변수 이름과 예시 값만 기록하고 실제 키를 입력하지 않습니다.
...
GitHub에 올리기 전에는 다음 명령으로 어떤 파일이 커밋 대상인지 확인합니다.

```bash
git status
```

`.env`가 이미 Git의 추적 대상인지 확인하려면 다음 명령을 사용할 수 있습니다.

```bash
git ls-files .env
```

명령 실행 결과에 `.env`가 표시되면 `.gitignore`에 추가하는 것만으로는 추적이 중단되지 않습니다. 실제 파일은 로컬에 남겨 둔 채 Git의 추적 대상에서만 제거합니다.

```bash
git rm --cached .env
```

그런 다음 다시 `git status`로 상태를 확인하고 변경 사항을 커밋합니다. API 키가 원격 저장소에 한 번이라도 올라갔다면 파일을 삭제하는 것만으로는 충분하지 않습니다. 해당 키를 즉시 폐기하거나 재발급하고, 필요하면 Git 기록에서도 비밀정보를 제거해야 합니다.
